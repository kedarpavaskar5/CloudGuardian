...
Eleven `.tf` files, all secure-by-default, with **eighteen boolean toggles**
...
All eighteen default to the **secure** setting. Set any of these to `true`
...
| `misconfig_sql_disable_tde` | Transparent Data Encryption turned off on the SQL database | Encryption |
| `misconfig_sql_min_tls_version` | SQL server minimum TLS lowered to 1.0 | Encryption |
...
| `misconfig_short_log_retention` | Log Analytics retention shortened from 30 to 7 days | Logging |
...
| `variables.tf` | All inputs, including all 18 misconfig toggles |
